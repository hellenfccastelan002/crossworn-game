...
const updateScore = () => {
    ...
    if (score >= 100 && !backgroundChanged) {
        ...
        pipe.src = "./imagem/pipe_vermelho.png";
...
const loop = () => {
    ...
    if (monstroLeft + monstro.offsetWidth - 20 >= marioLeft && monstroLeft <= marioLeft + mario.offsetWidth - 20 && marioBottomPosition < 80) {
        handleGameOver({pipe: pipePosition}, marioLeft); 
    }

    else if (pipePosition < -100 && gameStarted) { 
        pipe.style.left = ''; 
        updateScore(); 
    }
}
